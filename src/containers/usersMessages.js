               ...
               <div className="row">
                   <div className="col-2">
                       <div className="nav flex-column nav-pills" id="v-pills-tab" role="tablist"
                           ...
                           {users.map(user => {
                               return <a key={user.id} className="nav-link" id={`v-pills-${user.id}-tab`} data-toggle="pill" href={`#v-pills-${user.id}`}
                                         role="tab" aria-controls={`v-pills-${user.id}`} aria-selected="true">{user.firstName}</a>
                           })}
                   ...
                   <div className="col-4">
                       <div className="tab-content" id="v-pills-tabContent">
                           {users.map(user => {
                               return <div key={user.id} className="tab-pane fade" id={`v-pills-${user.id}`} data-toggle="pill" role="tabpanel"
                                          aria-labelledby={`v-pills-${user._id}-tab`} aria-selected="true">{user.user_messages.map( (message, index) => {
                                   ...
                                   <button className="btn btn-primary" onClick={() => this.onSendMessage({chatId: user.chatId, userId: user.id})}>Submit</button>
                               </div>
                           })}
